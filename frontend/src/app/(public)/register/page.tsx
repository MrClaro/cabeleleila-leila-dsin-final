...
export default function Registrar() {
	...
	async function registrar(ev: React.FormEvent<HTMLFormElement>) {
		ev.preventDefault();
		try {
			console.log(`http://${HOST}:${PORT}/api/registrar`);
			const response = await fetch(`http://${HOST}:${PORT}/registrar`, {
				method: "POST",
				body: JSON.stringify({ nome: usuario, email, telefone, senha }),
			...
			if (response.ok) {
				...
				setErrorMessage(
					errorData.message || "Por favor preencha todos os campos",
				);
				setIsErrorDialogOpen(true);
			}
		} catch (error) {
			console.error("Registro falhado:", error);
			setErrorMessage("Um erro ocorreu ao tentar fazer registro");
			setIsErrorDialogOpen(true);
		}
	}
	...
	return (
		<div className="max-w-md w-full mx-auto rounded-none md:rounded-2xl p-4 md:p-8 shadow-input bg-white dark:bg-black">
			...
			<form className="my-8" onSubmit={registrar}>
				...
				>
					<span>Já tem uma conta? logue já</span>
				</Link>
				<Link
					href="/"
